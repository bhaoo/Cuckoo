...
function parseBiaoQing($content){
  ...
  $content = preg_replace_callback('/\:\s*(huaji\d{1,2})\s*\:/is',
...
function parsePaopaoBiaoqingCallback($match){
  return '<img class="emoji-img-tieba" src="'.staticFiles('/assets/images/OwO/tieba/'.$match[1].'.png').'">';
 }
function parseHuajibiaoqingCallback($match){
  return '<img class="emoji-img-hj" src="'.staticFiles('/assets/images/OwO/huaji/'.$match[1].'.gif').'">';
 }
